...
export default function Onboarding() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-sage-300/10 to-mist-300/10 flex flex-col items-center justify-between px-6 py-16 safe-top">
      ...
      <div className="flex flex-col items-center text-center gap-5">
        ...
        <p className="text-stone-400 text-sm leading-relaxed max-w-xs">
          A quiet space for your mind. We'll help you find calm, sleep better, and build a practice that fits your life.
        </p>
      </div>
      <button
        onClick={() => setStep(1)}
  ...
  if (step === 1) return (
    <div className="min-h-screen bg-cream-50 flex flex-col px-6 py-14 safe-top">
      <div className="mb-2">
        <p className="text-[10px] font-semibold text-stone-300 tracking-widest uppercase">1 of 2</p>
        ...
        {GOALS.map(g => (
          <button
            ...
            className={`w-full rounded-3xl p-4 text-left flex items-center gap-4 transition-all active:scale-[0.98] shadow-soft
              ${goal === g.id ? 'bg-sage-400' : 'bg-white'}`}
          >
      ...
      <button
        ...
        disabled={!goal}
        className="mt-6 w-full bg-stone-800 text-white rounded-2xl py-4 font-medium text-sm disabled:opacity-30 active:scale-[0.98] transition-all"
      >
        Continue
      </button>
    </div>
  ...
  return (
    <div className="min-h-screen bg-cream-50 flex flex-col px-6 py-14 safe-top">
      <div className="mb-2">
        <p className="text-[10px] font-semibold text-stone-300 tracking-widest uppercase">2 of 2</p>
        ...
        {EXPERIENCE.map(e => (
          <button
            ...
            className={`w-full rounded-3xl p-4 text-left transition-all active:scale-[0.98] shadow-soft
              ${experience === e.id ? 'bg-sage-400' : 'bg-white'}`}
          >
